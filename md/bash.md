...
```bash
                                                # выполнить <command> с перенаправлением потоков
find ... -exec bash -c '<command> >> "{}"' \;   # файлы, содержащие пробелы в имени будут обработаны корректно
```
...
HOME=<dir>                        # изменение/создание переменной (для всей сессии)
export HOME=<dir>                 # изменение/создание переменной (для всей сессии)
export HOME=<dir> NEW_VAR=<value> # изменение/создание переменных (для всей сессии)
...
![информация о пользователях](https://github.com/cgehuzi/notes/raw/main/images/bash-cat-passwd.jpg)
...
![информация о правах доступа](https://github.com/cgehuzi/notes/raw/main/images/bash-permissions.jpg)

**`r — read`** : право на чтение<br>
**`w — write`** : право на запись (редактирование)<br>
...
chmod 644 <file>                # изменение для всех у файла (числовые значения)
...
chmod 644 <dir>                 # изменение для всех у директории (числовые значения)
